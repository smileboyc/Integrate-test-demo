...
const Admin: React.FC = () => {
  ...
  const handleAddHotel = (e: React.FormEvent) => {
    ...
    if (!hotelForm.name || !hotelForm.location || !hotelForm.price) {
      setSnackbar({ open: true, message: 'Please fill in all required hotel fields.' });
      return;
    }
    fetch('/api/admin/add', {
      method: 'POST',
  ...
  const handleAddFlight = (e: React.FormEvent) => {
    e.preventDefault();
    // Validation
    if (!flightForm.airline || !flightForm.from || !flightForm.to || !flightForm.price) {
      setSnackbar({ open: true, message: 'Please fill in all required flight fields.' });
      return;
    }
    fetch('/api/admin/add', {
      method: 'POST',
