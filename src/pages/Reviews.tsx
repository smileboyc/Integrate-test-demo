...
const Reviews: React.FC = () => {
  ...
  useEffect(() => {
    // Pre-select hotel/flight if coming from booking
    if (location.state && (location.state as any).type && (location.state as any).itemId) {
      if ((location.state as any).type === 'hotel') {
        setTab(0);
        setSelectedHotel((location.state as any).itemId);
      } else if ((location.state as any).type === 'flight') {
        setTab(1);
        setSelectedFlight((location.state as any).itemId);
      }
    }
  }, [location.state]);
